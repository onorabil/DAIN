...
psnr_array=np.zeros(len(file_list))
mse_array=np.zeros(len(file_list))
...
for idx, current_file in enumerate(files_in[:-1]):
    ...
    #if idx % 10 ==0:
    #    cv2.imwrite('out_flow_translated_pytorch_idx_{}.png'.format(idx), out_image[:,:,::-1])
    #    cv2.imwrite('out_flow_image_1.png_idx_{}.png'.format(idx), imgR_o[:,:,::-1])

    gt_rgb = np.array(imgR_o, dtype=np.float32)
    rec_rgb = np.array(out_image.astype(numpy.uint8), dtype=np.float32)
...
np.save('out_mse_test.npy', mse_array) 
