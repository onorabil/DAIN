...
def image_translated_with_flow_torch(rgb_1,flow):
    ...
    flow_stacked_neutral = np.meshgrid(np.linspace(-1,1,flow.shape[1]), np.linspace(-1,1,flow.shape[0]))
...
arguments_strOut = "out_flow_image_0_interpolated.png"
...
cv2.imwrite(os.path.join(OUTPUT_PATH, 'out_flow_translated_pytorch.png'), image_translated_with_flow_torch(imgL_o,offset[1])[:,:,::-1])
cv2.imwrite(os.path.join(OUTPUT_PATH, 'out_flow_image_0.png'), imgL_o[:,:,::-1])
cv2.imwrite(os.path.join(OUTPUT_PATH, 'out_flow_image_1.png'), imgR_o[:,:,::-1])
